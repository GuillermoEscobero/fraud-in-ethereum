...
def get_address_blacklist():
    f = open(BLACKLIST_YAML_FILE, "r")
    address_blacklist = yaml.load(f, Loader=yaml.FullLoader)
    f.close()

    return address_blacklist

def get_address_whitelist():
    f = open(LICIT_YAML_FILE, "r")
    address_whitelist = yaml.load(f, Loader=yaml.FullLoader)
    f.close()

    return address_whitelist

def get_total_number_of_normal_transactions(data):
    num_normal_transactions = 0

    if data['status'] != 0:
        num_normal_transactions = len(data['result'])

    return num_normal_transactions

def get_features_from_address(address, flag):
